...
%% \item {\it basketcache}: caches decompressed basket data, to avoid cache-misses when slicing or interpreting different ways.
...
%% \vspace{0.25 cm}
%% Even though Python has a global interpreter lock (GIL), most of the work is performed in compiled code with the GIL released.
...
%% \vspace{0.25 cm}
%% Returns immediately and {\it does no work at all} until/unless you ask for items.
...
%% \vspace{0.25 cm}
%% Hint: use with caching to avoid re-reading when called twice. Nothing is implicit!
%% \begin{minted}{python}
%% >>> read_only_once = t.lazyarrays(cache={})
%% \end{minted}
